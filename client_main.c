...
int main(int argc,char **argv)
{
	...
	{
		printf("signal SIGALARM failed:%s\n",strerror(errno));
	}
	...
	alarm(3);//定时开始，超时采样，采样结束上传服务器
	while(!get_stop)
	{
		// DS_get_tem(&temp,chip);
		if(sampleOver_statu == 1)
		{
...
{
	printf("programer will be stop...\n");
	get_stop = 1;
}

void sig_timer(int signo)
{
	printf("pi start sample...\n");
	DS_get_tem(&temp,chip);
	sampleOver_statu = 1;
	alarm(3);
}
